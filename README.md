...
A map is an object that defines highlighted areas on the image.

Its structure is similar to HTML's image map syntax:

- **map**: (_object_) Object representing the highlighted zones.
  - **name**: (_string_) The name of the map, used to associate it with the image.
  - **areas**: (_array_) Array of **area objects**, each with the following structure:
